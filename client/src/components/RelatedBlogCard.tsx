...
export const RelatedCard = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const res = await axios.get<Blog[]>(`/blog/userblogs`, { withCredentials: true });
        setBlogs(res.data);
  ...
  const handleUpvote = async (id: string, currentUpvote: number) => {
    try {
      if (!voted[id]) {
        await axios.post(`/blog/b/${id}`, { upvote: true }, { withCredentials: true });
  ...
  const handleDownvote = async (id: string, currentUpvote: number) => {
    try {
      if (!voted[id]) {
        await axios.post(`/blog/b/${id}`, { downvote: true }, { withCredentials: true });
        setLikeCounts((prev) => ({ ...prev, [id]: currentUpvote - 1 }));
        setVoted((prev) => ({ ...prev, [id]: true }));
      }
    } catch (e) {
